...
const Kwitansi = ({ documentId, projectDetailData, onCreated, currFileType, projectName }, ref) => {
  ...
  const [formData, setFormData] = useState({
    ...
    sumber_dana: "",
    tanggal_sumber_dana: "",
    nomor_surat_addendun: "",
    tanggal_surat_addendun: "",
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        await new Promise((resolve) => setTimeout(resolve, 300)); // Tambahin ini
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/dynamic/crud/get_by_id/kwitansi_tahap?id=${documentId}`);
        const rawData = response.data;

        setFormData({
          nomorSuratBAP: rawData.nomor_surat_bap || "",
          tanggalSuratBAP: parseDateString(rawData.tanggal_surat_bap) || null,
          tanggal_kwitansi: parseDateString(rawData.tanggal_kwitansi) || null,
          ...
          sumber_dana: rawData.nomor_surat_sumber_dana || "",
          tanggal_sumber_dana: parseDateString(rawData.tanggal_surat_sumber_dana),
          nomor_surat_addendun: rawData.surat_addendum || "",
          tanggal_surat_addendun: parseDateString(rawData.tanggal_surat_addendum),
        });
      } catch (error) {
    ...
    // Delay dikit supaya backend udah siap datanya
    const timeoutId = setTimeout(fetchData, 300); // 300ms

    return () => clearTimeout(timeoutId); // cleanup kalau documentId berubah lagi
  }, [documentId]);
  ...
  const handleSubmitKwitansi = async () => {
    ...
    const payload = {
      ...
      nomor_surat_sumber_dana: formData.sumber_dana,
      tanggal_surat_sumber_dana: formData.tanggal_sumber_dana ? format(formData.tanggal_sumber_dana, "yyyy-MM-dd") : null,
      uang_sebanyak: formData.total_uang.raw,
      uang_sebanyak_huruf: formData.total_uang_terbilang,
      untuk_pembayaran: formData.tujuan_pembayaran,
      surat_addendum: formData.nomor_surat_addendun,
      tanggal_surat_addendum: formData.tanggal_surat_addendun ? format(formData.tanggal_surat_addendun, "yyyy-MM-dd") : null,
      nomor_surat_bap: formData.nomorSuratBAP,
      tanggal_surat_bap: format(formData.tanggalSuratBAP, "yyyy-MM-dd"),
      tanggal_kwitansi: format(formData.tanggal_kwitansi, "yyyy-MM-dd"),
    };

    try {
      if (documentId) {
        // PUT (Update)
        const response = await axios.put(`${process.env.REACT_APP_BASE_URL}/dynamic/crud/put/kwitansi_tahap?id=${documentId}`, payload, { headers: { "Content-Type": "application/json" } });
        ...
        // POST (Create)
        const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/dynamic/crud/post/kwitansi_tahap`, payload, { headers: { "Content-Type": "application/json" } });

        const newId = response.data?.id;
        console.log("✅ Create success, new ID:", newId);

        if (newId) {
          // ⬇️ Langsung GET data baru
          const fetchResponse = await axios.get(`${process.env.REACT_APP_BASE_URL}/dynamic/crud/get_by_id/kwitansi_tahap?id=${newId}`);
          const fetchedData = fetchResponse.data;

          if (onCreated) {
            onCreated(newId, fetchedData); // kirim id + data ke parent
          }
        }
        const currentTimestamp = new Date().toISOString();
            ...
            <>
              <div className="kwitansi-content">
                ...
                <div className="kwitansi-detail-container">
                  ...
                  <span className="value-detail-kwitansi">
                    <span>
                      {formData.sumber_dana || "(Nomor Surat Sumber Dana)"},{" "}
                      {(formData.tanggal_sumber_dana &&
                        new Date(formData.tanggal_sumber_dana).toLocaleDateString("id-ID", {
                          day: "numeric",
                          month: "long",
                          year: "numeric",
                        })) ||
                  ...
                  <span className="value-detail-kwitansi">
                    ...
                    <span>
                      {formData.nomor_surat_addendun || "(Nomor Surat Addendun)"},{" "}
                      {(formData.tanggal_surat_addendun &&
                        new Date(formData.tanggal_surat_addendun).toLocaleDateString("id-ID", {
                          day: "numeric",
                          month: "long",
                          year: "numeric",
                        })) ||
                ...
                >
                  ...
                  >
                    <tbody>
                      ...
                      <tr>
                        <td style={{ paddingRight: "10px", whiteSpace: "nowrap" }}>Sumber Dana</td>
                        <td style={{ padding: "0 5px" }}>:</td>
                        <td>
                          {formData.sumber_dana || "(Nomor Surat Sumber Dana)"},{" "}
                          {(formData.tanggal_sumber_dana &&
                      ...
                      <tr>
                        <td style={{ paddingRight: "10px", whiteSpace: "nowrap" }}>Nomor Addendum</td>
                        <td style={{ padding: "0 5px" }}>:</td>
                        <td>
                          Nomor : {formData.nomor_surat_addendun || "(Nomor Surat Addendun)"},{" "}
                          {(formData.tanggal_surat_addendun &&
                            new Date(formData.tanggal_surat_addendun).toLocaleDateString("id-ID", {
                              day: "numeric",
                              month: "long",
    ...
    <>
      ...
      <div className="main-form-container">
        <div className="form-container">
          ...
          <div className="input-container-form">
            <label className="input-label-form">
              Nomor Surat Sumber Dana <span style={{ color: "red" }}>*</span>
            </label>
            <input
              type="text"
              name="sumber_dana"
              className={errors.sumber_dana ? "input-field-form-error" : "input-field-form"}
              ...
              onBlur={handleBlur}
            />
            {errors.sumber_dana && <span className="error-text">{errors.sumber_dana}</span>}
          </div>

          <div className="input-container-form">
            <label className="input-label-form">
              Tanggal Surat Sumber Dana <span style={{ color: "red" }}>*</span>
            </label>
            <DatePicker
              selected={formData.tanggal_sumber_dana}
              name="tanggal_sumber_dana"
              onChange={(date) => handleDateChange("tanggal_sumber_dana", date)}
              onFocus={handleFocus}
              onBlur={handleBlur}
              className={errors.tanggal_sumber_dana ? "input-field-form-date-error" : "input-field-form-date"}
              dateFormat="dd/MM/yyyy"
              placeholderText="Isi bagian ini"
            />
            {errors.tanggal_sumber_dana && <span className="error-text">{errors.tanggal_sumber_dana}</span>}
          </div>

          <div className="input-container-form">
            <label className="input-label-form">
              Nomor Surat Addendun <span style={{ color: "red" }}>*</span>
            </label>
            <input
              type="text"
              name="nomor_surat_addendun"
              className={errors.nomor_surat_addendun ? "input-field-form-error" : "input-field-form"}
              value={formData.nomor_surat_addendun}
              placeholder="Isi bagian ini"
              onChange={handleChange}
              onFocus={handleFocus}
              onBlur={handleBlur}
            />
            {errors.nomor_surat_addendun && <span className="error-text">{errors.nomor_surat_addendun}</span>}
          </div>
          <div className="input-container-form">
            <label className="input-label-form">
              Tanggal Surat Addendun <span style={{ color: "red" }}>*</span>
            </label>
            <DatePicker
              selected={formData.tanggal_surat_addendun}
              name="tanggal_surat_addendun"
              onChange={(date) => handleDateChange("tanggal_surat_addendun", date)}
              onFocus={handleFocus}
              onBlur={handleBlur}
              className={errors.tanggal_surat_addendun ? "input-field-form-date-error" : "input-field-form-date"}
              dateFormat="dd/MM/yyyy"
              placeholderText="Isi bagian ini"
            />
            {errors.tanggal_surat_addendun && <span className="error-text">{errors.tanggal_surat_addendun}</span>}
          </div>
        </div>
        {/* Word */}
        ...
        <ComponentWord />
        {/* UI */}
        <KwitansiPreview previewFormData={formData} detailDataProject={projectDetailData} />
      </div>
...
export default forwardRef(Kwitansi);
